...
buildpacks="\
build-system-buildpack,https://repo.spring.io/libs-snapshot-local/org/cloudfoundry/buildsystem/org.cloudfoundry.buildsystem/${VERSION}/org.cloudfoundry.buildsystem-${VERSION}.tgz \
debug-buildpack,https://repo.spring.io/libs-snapshot-local/org/cloudfoundry/debug/org.cloudfoundry.debug/${VERSION}/org.cloudfoundry.debug-${VERSION}.tgz \
...
for bp in ${buildpacks};do
  ...
  url=$(dirname ${url})
  file=$(curl -s ${url}/ | grep tgz | awk -F '<' '{print $2}' | awk -F '>' '{print $2}')
  sed -i.bak "s|${id}|${url}/${file}|g" builder.toml
done
